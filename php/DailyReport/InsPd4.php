...
//  $data_json = array_values($data_json); //配列の並び替え
  ...
  try {
      ...
      // $prepare->bindValue(':no1_work_length', Null, PDO::PARAM_STR);
      ...
      if ($_POST['directive_input__select'] == '') {
          $prepare->bindValue(':ordersheet_id', null, PDO::PARAM_STR);
      } else {
          $prepare->bindValue(':ordersheet_id', (INT)$_POST['directive_input__select'], PDO::PARAM_INT);
      }
      ...
      $prepare = $dbh->prepare("
      SELECT MAX(t_press.id) AS id FROM t_press
    ");
      $prepare->execute();
