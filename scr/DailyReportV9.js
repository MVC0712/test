...
// Directive date
$(document).on("keydown", "#press-directive__select", function(e) {
...
$("#add-rack__button").on("click", function() {
    ...
    switch ($(this).text()) {
        ...
        case "Update":
            ...
            $("#rack_selected__tr td:nth-child(3)").text(
                $("#rackqty__input").val()
            );
...
function ajaxInsPd(inputData) {
    ...
    $.ajax({
        ...
        .done(function(data) {
            sendTable.push(data["id"]); // targetIdのデータを挿入
            ajaxInsWorkInformation2(sendTable);
            if ($("#rack__table tbody tr").length >= 1) {
                // ラックの入力があるときだけ処理する
                // agintTableのデータの取得
                agingTable = getTableData($("#rack__table tbody tr"));
                ajaxInsUsedAgingRack(agingTable);
            }
...
function clearInputData() {
    $(".input-required").removeClass("complete-input").addClass("no-input");
    ...
    //   .addClass("complete-input")
    //   .focus();
    // work-length__table の3行目以降を削除する
    $("#work-length__table tbody tr").each(function(index, element) {
        if (index >= 2) {
            $(this).remove();
        }
    });
...
$(document).on("click", "#update__button", function() {
    ...
    ajaxUpdatePd(inputData); // t_press データ更新
    // console.log("-------hello");
    // return false;

    ajaxInsWorkInformation2(sendTable); // t_press_work_length_quantity データ更新
    ajaxUpdateUsedRack(tableData); // t_using_aging_rack データ更新
...
function ajaxDelSelData(targetId) {
    $.ajax({
            type: "POST",
            url: "./php/DailyReport/DelSelData3.php",
            dataType: "json",
            // async: false,
            data: {
                id: targetId,
...
function ajaxSelSelData(targetId) {
    $.ajax({
            type: "POST",
            url: "./php/DailyReport/SelSelData5.php",
            dataType: "json",
            // async: false,
            data: {
                targetId: targetId,
...
function fillReadData(data) {
    ...
    targetDom.eq(10).val(data[0]["no2_1000_ram_speed"]);
    targetDom.eq(11).val(data[0]["no2_1000_ram_pressure"]);
    targetDom.eq(12).val(data[0]["no2_1000_work_temperature"]);
    targetDom.eq(13).val(data[0]["no2_0200_ram_speed"]);
    targetDom.eq(14).val(data[0]["no2_0200_ram_pressure"]);
    targetDom.eq(15).val(data[0]["no2_0200_work_temperature"]);
...
function ajaxSelWorkInformation(targetId) {
    ...
    $.ajax({
            type: "POST",
            url: "./php/DailyReport/SelWorkInformation2.php",
            dataType: "json",
            // async: false,
            data: {
                id: targetId,
        ...
        .done(function(data) {
            ajaxSelRack($("#selected__tr").find("td").eq(0).html()); // aging rack infomation
            //
            // console.log(data);
            fillWorkInformation(data);
        })
        .fail(function() {
            alert("DB connect error");
        });
...
function ajaxInsUsedAgingRack(tableData) {
    // console.log(tableData);

    $.ajax({
            type: "POST",
            url: "./php/DailyReport/InsUsedAgingRack.php",
            dataType: "json",
            async: false,
...
function ajaxSelRack(targetId) {
    $.ajax({
            type: "POST",
            url: "./php/DailyReport/SelRack.php",
            dataType: "json",
            async: false,
            data: { id: targetId },
        })
...
function ajaxUpdateUsedRack(tableData) {
    console.log(tableData);
    $.ajax({
            type: "POST",
            url: "./php/DailyReport/UpdateUsedRack.php",
            dataType: "json",
            async: false,
            data: JSON.stringify(tableData),
        })
...
$(document).on("click", "#add_row__button", function() {
    var trDom = $("<tr>");
    var recordNumber = Number($("#work-length__table tbody tr").length) + 1;

    trDom.append($("<td>").html("No." + recordNumber));
    for (i = 0; i < 6; ++i) {
        var tdDom;
        // if (i % 2 == 0) {
...
$(document).on("click", "#test__button", function() {
    ...
    if ($("#rack__table tbody tr") >= 1) {
        // ラックの入力があるときだけ処理する
        ajaxInsUsedAgingRack(getTableData($("#rack__table tbody tr")));
    } else {
        console.log("hello");
    }
});
...
function makeSendData(workInfrmationTable) {
    ...
    workInfrmationTable.forEach(function(element, index) {
        sendTable.push([index + 1, 1, element[1], element[2]]);
        sendTable.push([index + 1, 2, element[3], element[4]]);
        sendTable.push([index + 1, 3, element[5], element[6]]);
    });
...
function ajaxInsWorkInformation2(tableData) {
    $.ajax({
            type: "POST",
            url: "./php/DailyReport/InsWorkInformation2.php",
            dataType: "json",
            async: false,
            data: JSON.stringify(tableData),
        })
...
$(document).on("click", "#test3__button", function() {
    clearInputData();
});