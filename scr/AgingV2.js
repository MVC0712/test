...
// deleteダイアログの add ボタンが押されたとき
$(document).on("click", "#delete-dialog-add__button", function () {
  editMode = "NEW_AGING";
  buttonActivation();
  moveAgingTr($("#not_aging__table__selected"));
  deleteDialog.close();
});
...
$(document).on("click", "#delete-dialog-delete__button", function () {
  deleteDialog.close();
  confirmDialog.showModal(); // 再度確認ダイアログを表示する
});

// confirmダイアログの cancel ボタンが押されたとき
$(document).on("click", "#confirm-cancel__button", function () {
  confirmDialog.close();
});

// confirmダイアログの delete ボタンが押されたとき
$(document).on("click", "#confirm-delete__button", function () {
  let targetId;
  targetId = $("#not_aging__table__selected td:nth-child(1)").html();
  ajaxDelAgingRack(targetId);
  confirmDialog.close();
});

function ajaxDelAgingRack(targetId) {
  $.ajax({
    type: "POST",
    url: "./php/Aging/DelAgingRack.php",
    dataType: "json",
    async: false,
    data: {
      id: targetId,
