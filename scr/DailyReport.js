...
$(function () {
  ...
  // 今日の日付の代入
  $('#date__input').val(fillToday()).removeClass("no-input")
    .addClass("complete-input").focus();
...
function setDate(e, targetDom) {
  ...
  setDate = (Number(dateLetters[0]) + 2000) + "/" + dateLetters[1] + "/" + dateLetters[2];
  ...
  displayDate = (Number(inputDay.getFullYear()) - 2000) + "-"
...
function fillToday() {
  // 本日の日付をyy-mm-dd形式で返す
  let dt = new Date();
  return ((dt.getFullYear() - 2000) + "-" + (dt.getMonth() + 1) + "-" + dt.getDate());
}
  ...
  $.ajax({
    ...
    .done(function (data) {
      ...
      data.forEach(function (value) {
        $("#die__select").append($("<option>").val(value["id"]).html(value["die_number"]));
      })
    })
    .fail(function () {
      alert("DB connect error");
...
$(document).on('keydown', '#billet-lot-number__input', function (e) {
  ...
  $("#billet-size__select").val("9").removeClass("no-input").addClass("complete-input");
})
...
function ajaxSelStop(inputValue) {
  ...
  $.ajax({
    ...
    .done(function (data) {
      ...
      data.forEach(function (value) {
        $("#stop-cause__select").append($("<option>").val(value["id"]).html(value["stop_code"]));
      })
    })
    .fail(function (data) {
      // alert("DB connect error");
...
function ajaxSelStaff(inputValue) {
  ...
  $.ajax({
    ...
    .done(function (data) {
      ...
      data.forEach(function (value) {
        $("#name__select").append($("<option>").val(value["id"]).html(value["staff_name"]));
      })
    })
    .fail(function (data) {
      // alert("DB connect error");
...
function clearInputData() {
  $("input.save-data")
    .val("").removeClass("complete-input").addClass("no-input");
  $("input.need-clear").val("");
  $("select.need-clear")
    .val(0).removeClass("complete-input").addClass("no-input");
}
// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
// ------------------------- Update BUTTON  ----------------------------------
// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
function ajaxUpdatePd(inputData) {
  $.ajax({
    type: "POST",
    url: "./php/DailyReport/UpdatePd.php",
    dataType: "json",
    // async: false,
    data: inputData,
  })
...
function ajaxSelSelData(targetId) {
  $.ajax({
    type: "POST",
    url: "./php/DailyReport/SelSelData.php",
    dataType: "json",
    // async: false,
    data: {
      targetId: targetId,
...
function fillReadData(data) {
  ...
  $("#die__select").empty().append($('<option>').html(data[0]["die_number"]).val(data[0]["dies_id"]));
  $("#stop-cause__select").empty().append($('<option>').html(data[0]["stop_code"]).val(data[0]["press_stop_cause_id"]));
  ...
  $("#name__select").empty().append($('<option>').html(data[0]["staff_name"]).val(data[0]["staff_id"]));
