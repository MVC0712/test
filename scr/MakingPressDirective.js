...
function setDate(e, targetDom) {
  ...
  let displayDate;
  let today = new Date();
  today.setDate(today.getDate() + 20); // 20日先まで入力出k理宇
...
$(document).on("change", "div.main__wrapper", function () {
  // console.log("hello:" + checkInputComplete());
...
function getInputData() {
  ...
  // 今日の日付
  inputData["created_at"] = "20" + fillToday();
  // 型番の取得
...
function ajaxUpdateData(inputData) {
  $.ajax({
    type: "POST",
    url: "./php/MakingPressDirective/UpdateData.php",
    dataType: "json",
    // async: false,
    data: inputData,
  })
...
function ajaxSelSelData(targetId) {
  $.ajax({
    type: "POST",
    url: "./php/MakingPressDirective/SelSelData.php",
    dataType: "json",
    async: false,
    data: {
      targetId: targetId,
